...
class Q(nn.Module):
    def __init__(self, state_dim, act_dim, n_blocks, h_dim, context_len,
        ...
        ### prediction heads
        self.predict_reward = torch.nn.Linear(2 * h_dim, 1)
        self.predict_rtg = torch.nn.Linear(2 * h_dim, 1)
    ...
    def forward(self, timesteps, states, actions):
        ...
        h = h.reshape(B, T, 1, 2 * self.h_dim).permute(0, 2, 1, 3)

        # get predictions
        return_reward = self.predict_reward(h[:, 0])
    ...
    def train_actor(self, timesteps, states, actions):
        with torch.no_grad():
            B, T, _ = states.shape
...
class actor(nn.Module):
    def __init__(self, state_dim, act_dim, n_blocks, h_dim, context_len,
        ...
        # 这里是直接不对action做区分，因为actor的输入只有state
        # # discrete actions
        # self.embed_action = torch.nn.Embedding(act_dim, h_dim)
        # use_action_tanh = False # False for discrete actions
    ...
    def forward(self, timesteps, states):
        ...
        #     (state_embeddings, action_embeddings), dim=1
        # ).permute(0, 2, 1, 3).reshape(B, 2 * T, self.h_dim)
        h = state_embeddings.reshape(B, T, self.h_dim)
        h = self.embed_ln(h)
        ...
        # get predictions
        # return_preds = self.predict_rtg(h[:,1])     # predict next rtg given r, s, a
        # state_preds = self.predict_state(h[:,1])    # predict next state given r, s, a
        action_preds = self.predict_action(h[:, 0])  # predict action given r, s
...
class DC(nn.Module):
    def __init__(self, state_dim, act_dim, n_blocks, h_dim, context_len,
        ...
        self.reward_to_action = torch.nn.Linear(h_dim, h_dim)
        self.predict_action = nn.Sequential(
            nn.Linear(h_dim, act_dim),
            nn.Tanh()
        )

    def forward(self, timesteps, states, actions, rewards):
        B, T, _ = states.shape
        ...
        normalize_reward = torch.mean(normalize_with_l2(reward_embeddings))

        h = torch.stack(
            (state_embeddings+reward_embeddings, action_embeddings), dim=1
        ).permute(0, 2, 1, 3).reshape(B, 2 * T, self.h_dim)
        ...
        next_state_preds = self.predict_state(h[:, 1])  # reward influence in state prediction
        next_reward_preds = self.predict_rtg(h[:, 1])  # reward influence in rtg prediction
        action_preds = self.predict_action(h[:, 0]*normalize_reward)  # reward influence in action prediction

        return action_preds, next_state_preds, next_reward_preds
